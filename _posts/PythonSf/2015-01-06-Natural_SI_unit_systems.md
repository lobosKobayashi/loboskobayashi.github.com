...
1. ε0・μ0 は本質的に無次元の 1 である。それにも関わらず MKSA 単位系を採用すると、ε0・μ0 は組み立て単位を持ってしまう。SI 単位系では、1 の値にもなれなくなってしまう。
...
そして、SI 単位系・自然単位系・Planck 単位系・Heaviside-Lorentz 単位系全体を矛盾無く計算可能なプログラムの設計と実装することは、電磁単位系を誤りなく理解するための絶好の演習問題になっています。この四つを体系的に網羅した、丸写しするだけで四つの単位系全部を動かせてしまうような教科書が存在しないからです。循環論法による辻褄合わせレベルでの理解であれば、プログラム動作エラーとなってしまうからです。電磁単位系全体を正しく理解してないと、プログラムの実装とは独立して設けるプログラムのテスト・コードと実装プログラムの両者を矛盾無く動作させられないからです。
