...
おっちょこちょいな技術者 a careless engineer
調整定数 fixing up constant
-->
...
私に言わせれば "これにより" より後ろの文章は無茶苦茶です。磁気定数とはなんですか。自由空間の透磁率とは何ですか。論理が飛躍しすぎです。しかも SI 単位系の規格書には、磁気定数や自由空間の透磁率の言葉はここに出てくるだけで、その定義・説明がありません。 また μ0 の組み立て単位は Newton/A^2 または (Newton/m)/(A^2/m) とすべきです。この単位電流の定義を元に、H：ヘンリーも含めた組み立て単位を構築しなければならないのに、電流という基本単位を定義している段階で H という組立単位を入れてくるなんて循環論法です。

もっと基本に立ち返ります。ガウス単位系では ε0・μ0 は無次元の 1 の物理量でした。無次元だった物理量が、基本単位系のとり方を変えるだけで、MKSA 単位系に変えるだけで次元をもった物理量に変わるなんてありえないでしょう。自然法則は人間が採用する単位系に依存することなく同じように記述されるはずです。

さらに基本的なことを言います。そもそも透磁率という率の付いた物理量は無次元の物理量に使う言葉です。物理の次元をもった μ0 には透磁度の言葉を使うべきです。こんなことは電磁気学学の教科書を書くような学者ならば百も承知のはずです。根本的な何かが見逃されてそうです。<!-- 英文では省略する。 gaus system of units での透磁率・誘電率が無次元であることを説明することは上でなされている。-->

(この単位電流・μ0 の定義は[後で述べるようなものに](#definition_of_the_unit_current)修正すべきです。)

この考察では以下のことを示します。

1. 上の SI 単位系への批判・疑問が正当な物である。
1. SI 単位系での ε0・μ0 は人間の歴史的経緯によって定まった、物理的必然性のない値だ。[ε0 はここで詳述される物理的意味]()を持つ。[μ0 はここで詳述される物理的意味]()を持つ。
...
1. 分かりにくい・誤解されやすい MKSA 単位系を選択せざるをえなかった理由を、[SI 単位系：MKSA 単位系となった歴史的経緯](#why_mksa_unit_system)に示します。

ただし この論考を進めるには、SI 単位系は不適切です。電磁気自体の複雑さの上に、さらに歴史的経緯による複雑が重なってしまっているからです。より単純で見通しの良い自然単位系を主に使います。幸いにも [PythonSf](http://loboskobayashi.github.io/pysf/manual/one-liners.htm) では「ユーザー・カスタマイズ例として配布している Python モジュール・ファイル sfCrrntIniRelativity.py sfCrrntIniOpRelativity.py」に、自然単位系やプランク単位系を実装しています。これを使うことで、単位系の考察に出てくる数式を実際に計算可能なものにできます。論考を数値実験で確認していけます。

そもそも循環論法に陥ることなく電磁系の世界の組み立て単位全てを基本単位の組み合わせの連鎖として構築するには、電磁系の世界は複雑すぎます。コンピュータの助けを借りて初めて理解可能な複雑さになっています。
ですから、SI 単位系・自然単位系・Planck 単位系・Heaviside-Lorentz 単位系全体を矛盾無く計算可能なプログラムを設計・実装することは、電磁単位系を正く理解するための絶好の演習問題になっています。循環論法に目を瞑ったまま、辻褄合わせで電磁気の単位系を理解できた気になっているままで、この四つの電磁単位系のプログラムを実装すると、プログラムに循環参照ができてしまいます。無理に動かしても未定義エラーとなってしまいます。電磁単位系全体を正しく理解していないと、この四つの電磁単位系を実際に矛盾なく動作させるプログラムを実装できません。


以下の考察により、MKS:長さ・質量・時間の三つで記述される機械系の世界に、電磁気の物理世界が斜めに捻くれて組み合わされていることを示します。これが「根本的に見逃されていた何か」です。この論考の副産物として、解説されることの少ない微細構造定数の物理的意味を、
[h`` と c` と単位電荷と微細構造定数](#fine_structure_constant)で説明します。 

<blockquote>
<div class="qtitle">PythonSf の使用</div>
<br>
...
[Windows 版だけですが こちらにポータブル版の PythonSf](http://drive.google.com/drive/folders/0BwB_czqjpb6XaXF6TEluWWg1Ulk)を置いてあります。これを使えばインストール作業さえ無しで、PythonSf one-liners 数式を扱えます・計算・数値実験できます。批判も歓迎しますが、思い込みや歪曲での批判ではなく、PythonSf 式でも記述される数値実験可能で具体的な批判にしてください。矛盾点があれば、コンピュータ計算が可能な PythonSf 式として矛盾点を記述できるはずです。
...
##<a name="PythonSf_and_natural_unit_system"> PythonSf と自然単位系と組立単位の導出

>PythonSf では sfCrrntIniRelativity.py/sfCrrntIniOpRelativity.py の名前のファイルも配布しています。カレント・ディレクトリにあります。このファイル名を sfCrrntIni.py/sfCrrntIniOp.py に変更してやると、PythonSf で自然単位系を扱えるようになります。(既存の sfCrrnitIni.py sfCrrntIniOp.py は別のファイル名に変更してください。またこのディレクトリは単位系専用のディレクトリとしてください。)

自然単位系は光速度:c\` とプランク定数:h\`\`:\\(\hbar\\)(\\(2\pi\\) で割られたほう) と、1 electron volt を 1 となるように基本単位を選択した単位系です。相対論や量子力学での計算が楽になります。
...
数値実験を行うとき、コンピュータ計算での浮動小数点誤差が付きまといます。光速度のような大きな数やプランク定数のような小さな数を入れたまま浮動小数点計算をすると計算誤差がいちじるしく増加します。その結果、浮動小数点がもたらす計算誤差を含んだ正しいらしい値なのか、それとも元の数式が誤っていたのかの判別が困難になります。相対論や量子論での数値実験・浮動小数点計算を行うためには、自然単位系が必須となります。

数値計算のためだけでなく、電磁単位系を考察していく際にも自然単位系のほうが楽です。SI 単位系では天下りに与えられた・または辻褄あわせでしか導けなった ε0,μ0 の物理次元・単位が、Coulomb の法則・電磁誘導の法則から物理的必然として明晰に導かれます。SI:MKSA 単位系では物理的意味の不明な電圧組み立て単位：kg m^2/(A s^3), 抵抗組み立て単位：kg m^2/(A^2 s^3) を、辻褄あわせではなく、物理的な意味を伴った必然の物理量として組み立てられます。導出できます。

配布している PythonSf の sfCrrntIniRelativity.py/sfCrrntIniOpRelativity.py ファイルの中で自然単位系での四つの基本単位・組み立て単位を接尾辞に n を付加することで下のように定義しています。

- **基本単位** 単位長:mn\`, 単位質量:gn\`, 単位時間:sn\`, 単位電荷:Cn\`
- **組立単位** 単位力:Nn\`, 単位仕事:Jn\`, 単位電圧:Vn\`, 電磁系単位電圧:dVn\`, ε0:e0n\`, μ0:u0n\`, プランク定数(2pi で割った量):hn\`\`, 光速度:cn\`
- **素電荷** pQn\`

素電荷を除き これらの変数値は数値の 1 にすぎません。そうなるように基本単位量を選択したのですから。
...
自然単位系の物理量を toSI(..) 関数の引数値に与えて呼び出すと SI 単位系での単位も付いた物理量値が返ってきます。下のような具合です
...
tt() 関数を呼び出さないと、mn` gn` sn` Cn` eVn` Nn` Jn` Vn` dVn` e0n` u0n` hn`` cn` は単位を伴わない単なる数値 1 の変数ラベルになっています。その様な変数ラベルであったとしても、物理的に意味を持った PythonSf 式を書くときに、変数ラベルを追加するだけで物理的意味を明示できるので有用です。Python 関数の多くは sympy 単位属性をもった変数を引数に与えるとエラーになることが多いので、そのときは上の単位系を伴わないラベルを使わねばなりません。一方で PythonSf 基本関数および その組み合わせだけならば、sympy 単位属性を持っていても計算できます。ただし sqrt(..) 以外の PythonSf 基本関数に与える単位付きの変数は無次元の物理量に戻る式値でないとエラーになります。
...
<pre>
    ...
    tt(); v=0.8 mn`/sn`; sin(v/cn`), type(v/cn`)
    ===============================
    (0.71735609089952279, <class 'sympy.core.numbers.Float'>)
    ...
    # 0.8 mn`/sn` を光速度で割った値は、物理的には無次元の 1 ですが sympy 単位値であり
    # numpy.sin からの呼び出しではエラーになります。
...
以下、この PythonSf 自然単位系の計算機能を使って数値実験しながら、また物理的意味を明確にしながら、ε0, 電圧:kg m^2/(A s^3), 抵抗:kg m^2/(A^2 s^3), μ0 の組み立て単位を順を追って導出していきます。SI:MKSA 単位系では、これらは天下りに与えられ、また辻褄合わせで出てくるだけでした。

以下の考察は実際に PythonSf で計算可能な式を使って行っていきます。皆様もぜひ自分でも計算しながら読んでいっててください。自分で自分自身に問題提義しながら、ここでは書いていないことも含めて、御自分の頭でも考えながら読み進めてください。誤りがあったら、コンピュータの計算結果が想定外の値を返すので容易に気付けます。単位付きの加減乗除算を行うだけですから、[PythonSf マニュアル](http://loboskobayashi.github.io/pysf/manual/readMeOfPythonSf.htm)の詳細を読まなくても[ vim エディタで数式文字列を入力できさえすれば計算できます](http://loboskobayashi.github.io/pysf/manual/one-liners.htm#■■ PythonSf vim operation)。


###<a name="drive_e0"> クーロン力と 単位電荷・単位力と ε0
...
でも同時にクーロンの法則を使っても力を定義できます。単位距離だけ離れた単位電荷の間には **単位力/(4pi)** の力が働きます。ですから下の式・組み立て単位も力を示と看做せます。(注意: Cn^2/mn^2 が力の組立単位であるとは教科書には書いてありません。もし そのような教科書があったら教えてください)
...
これは基本単位系に冗長性があるためです。単位長さ:m と単位質量:g と単位時間:sの他に単位電荷:C の四つの基本単位からなる四元単位系:mgsCを採用したことで、単位系に冗長性が発生したのです。そのため力を現すのに g m s^-2 の組み立て単位の他に、クーロンの法則から導かれる C/m^2 も力の組み立て単位になってしまったわけです。(冗長さを伴わない mgs の三元系のまま電磁気量を組み立てていくことも可能です。ただし、そのような単位系では組み立て単位に 1/2 べき乗の量が入り込んでしまいます。[そのような Heaviside-Lorentz 単位系](#Heaviside_Lorentz_ing)も後で考察・検討します。また PythonSf に実装してあります。)

ここで下のように「電磁系の力の組立単位:C/m^2」から「機械系の力の組み立て単位:g m s^-2」への：二種類の力の単位換算定数 e0 を導入してやれば二つの力を互いに変換できます。
    ...
    # 単位 Coulomb 力を電気定数：ε0 で割って機械系での単位力に変換する
    tt(); (Cn`^2/mn`^2) /e0n`
    ===============================
    gn`*mn`/sn`**2
...
そして実際に SI:MKSA 単位系での電気定数：ε0 の組み立て単位も、上と同様に実際に Coulomb 力と機械系の力：F=m α の比になっています。下のような具合です。
    ...
    # 上の e0` の組立単位は下のように電磁系の世界のForce/機械系の世界のForce の比の式に変形できます。
...
MSAV 四元単位系では、ε0 は A`**2*s`**4/(kg`*m`**3) と物理的には同じだけど組み立て方が異なる A`*s`/(V`*m`) 表記になるため、 jsw` を掛けたり割ったりすることで、MKSA 組み立て単位に、ユーザーが自分で明示的に変換します。
</blockquote>

####<a name="rate_of_permittivity_of_vacuum"> 真空の誘電「率」
<!-- 英語版では rate の話をする
-->

SI 単位系では e0 のことを **真空の誘電「率」** と呼びます。率という漢字は無次元の量に使います。単位を持った物理量ならば **真空の誘電「度」** と言うべきです。でも以上の議論で示されるように ε0 は、同じ単位力の物理量 C/m^2 と g m s^-2 の換算定数にすぎません。本質的に無次元の 1 の量です。ε0 を **真空の誘電「率」** と電磁気学の教科書で書いてあることは正しいのです。SI 単位系では、歴史的経緯のせいで 1 にできなかっただけなんです。
...
同じ 物理量:力どうしの変換係数ですから自然単位系で 1 の量である e0n が SI 単位系での真空の誘電率：電気定数： e0 に正しく変換されます。このことからも SI 単位系での電気定数も本質的に無次元の 1 であることが解ります。(私自身は上の e0n の導出の仕方と、この e0n と e0 が同じ物理量であるがゆえに toSI(..) 関数で変換できることを知って、やっと電気定数:e0 の物理的意味を理解できた気になれました。皆様は如何でしょうか？)
...
###<a name="drivation_of_Volt_drived_unit"> 電圧の単位:gn`*mn`**2/(Cn`*sn`**2)：kg m^2/(A s^3) の導出
...
これにより SI 単位系での訳の解らない下の電圧単位：kg m^2/(A s^3) が導けました。C を A s で置き換えれば下の電圧単位になります。
    ...
    import sympy.physics.units as un; tt(); un.V, toSI(Vn`)
    ===============================
    (kg*m**2/(A*s**3), 0.302824523103951*kg`*m`**2/(A`*s`**3))

「V I == W の関係式から電圧の組み立て単位 kg m^2/(A s^3) を導出した」とし、SI 単位系での電圧の組み立て単位になんらの疑問を持たない方が多くいます。大多数の電気・回路技術者に「100 kg m^2/(A s^3) の電圧」と言っても通じないのにも関わらず平気でいられる方たちです。でも私は SI 単位系での電圧の組み立て単位：kg m^2/(A s^3) を理解できない方のほうが正しいと主張します。電圧の組み立て単位を導出しようとしているのに、 V I == W の関係式を持ち出すのは循環論法です。V I == W の関係式は電圧組み立て単位を導出した後に、下のような順序で導出される関係式だからです。

    # V I == W の導出
    V q == Energy → 両辺を時間で微分する→ V dq/dt = V I = dE/dt = W

「V I == W の関係式から電圧の組み立て単位 kg m^2/(A s^3) を導いた」のは、導出ではなく単なる辻褄合わせ：fitting だと私は主張します。単なる fitting で導いた「kg m^2/(A s^3) の電圧組立単位」が電気・回路技術者に通じないのは当然だと主張します。

大多数の電気・回路技術者は、そして物理学者の多くも、上で述べたような基本単位電荷から電圧の組み立て単位の導出を行っていません。MKSA 単位系では、電荷ではなく電流を基本単位とするからです。電流を基本単位とする単位系では上のように ε0 を組み立てるのは困難だからです。ですから、100 kg m^2/(A s^3) の電圧と言われても 100V の意味だと理解できません。

普通のエンジニアに「100 kg m/s^2/m^2 の圧力」「100 N/m^2 の圧力」「100 Pa の圧力」いずれの組み立て単位で圧力を表現しても通じるでしょう。もし「100 kg m/s^2/m^2 の圧力」で通じなかったら、その技術者は圧力という物理概念を理解していないことを意味します。でも「100 kg m^2/(A s^3) の電圧」が通じない技術者・物理学者が電圧の概念を理解していないわけではありません。彼らは電圧を組立単位ではなく基本単位として理解しているからです。彼らは質量ではなく電圧を基本単位に使う MSAV 四元単位系で閉じた電磁系の世界で理解・検討・設計しているからです。彼らにとって抵抗の組立単位は kg m^2/(A^2 s^3) ではなくV/A だからです。この詳細は[PythonSf MKS + A V 単位系](#MKSAV_unit_system)の章で論考します。
...
###<a name="drivation_of_ohm_drived_unit"> 抵抗の組み立て単位：kg m^2/(A^2 s^3) の導出
...
###<a name="rate_of_permeability_of_vacuum"> μ0n 組み立て単位の導出
...
##<a name="fine_structure_constant"> h`` と c` と単位電荷と微細構造定数
...
####<a name="fine_structur_constant_and_unit_charge"> 微細構造定数と単位電荷
...
修正自然単位系は光速度:c とプランク定数:\\(\hbar\\)と 1 electronVolt と素電荷を 1 となるように基本単位たちを選択した単位系です。修正自然単位系では ε0,μ0 を 1 にできません。逆に言えば、SI:MKSA 単位系で天下りに与えられていた、また歴史的経緯に妥協した、物理的には必然性のない真空の誘電率・透磁率:ε0,μ0 が、この単位系では必然性を持って出てきます。

自然単位系・Planck単位系のように e0n, e0p が 1 となる単位系では ε0,μ0 は単なる単位の換算定数にすぎず、本質的に無次元の 1 でしかありません。ε0,μ0 の新しい変数名を追加する強いモチベーションは生まれません。一方で物理を追求する意味では素電荷を単位電荷とする単位系が欲しくなります。なんせ素電荷が最小単位であり、それの N 倍の電荷量しかありえないのですから。しかし素電荷を単位電荷とする代償として ε0,μ0 の値を 1 に保てなくなります。微細構造定数は \\(\hbar\\) とc が 1 の単位系ならば、ε0 が 1 のとき、その単位電荷を定めてしまうからです。
...
直前の e0k の導出手順・考え方を単位電流の定義にも使うと、下の SI 単位系での単位電流と μ0 の定義は次のようにすべきだと分ります。

>電流の単位アンペアは，真空中に1 メートルの間隔で平行に配置された無限に小さい円形断面積を有する無限に長い二本の直線状導体のそれぞれを流れ，これらの導体の長さ1 メートルにつき2 × 10^-7 ニュートンの力を及ぼし合う一定の電流である．これにより，磁気定数μ0 ≡ 4π × 10^-7 (N/m)/(A^2/m) となる．

上の定義の物理的意味を、次のように明確にできます。

1. 距離 d だけ話して電流 I を流している二本の無限に平行な電線の間には I^2/d に比例する単位長さの力が働きます。
2. J. C. Maxwell が実用単位として定めた 1A の電流を流すと、1m だけ離れた二本の平行電線の間には 2x10^-7 N`/m` の力が働きます。これを単位電流の定義にします。[(この 10^-7 の係数は emu 単位系での単位電流:「1cmだけ離れた二本の平行電線の間に 2 dyne/cm の力が働く電流」の 10^-1 を1A:実用単位電流としたことから定められました)](#why_mksa_unit_system)
...
1. 単位電流を基本単位に追加したことで A^2/m も力を表す組み立て単位になってしまった。この力は 2x10^-7 N/m の力だ。この二つの力は組立単位は異なるが同じ力という物理量を表しているとみなせる。
1. この二つ力の単位換算定数 μ0 を導入して磁気定数の名前にしよう。ただし有理化単位系にしたいので平行電線間にはたらく単位長さ当たりの力の公式は 2 μ0 I^2/(4pi d) となるようにしよう。
1. 2 10^-7 N/m == 2 μ0 A^2/(4pi m) なのだから μ0 ≡ 10^-7(N/m)/(A^2/(4pi m)) となる。
1. ここで μ0 の単位として N/(A^2/m) が出てきたが、分母と分子はどちらも力の組み立て単位であり、物理的に無次元の量だ。単位の冗長性により物理次元を持っているにすぎない。この意味で「透磁**率**」の用語は適切だ。
...
実際、次のように PythonSf 式で u0 と 10^-7(Newton/meter)/(Ampere/(4pi 1meter)) が、値・単位の両方で同じであることを確認計算できます。

    ts(); u0`, u0` jsw`, 10^-7(N`/m`)/(A`^2/(4pi m`))
    ===============================
    (1.2566370614e-6*V`*s`/(A`*m`), 1.2566370614e-6*kg`*m`/(A`**2*s`**2), 1.25663706143592e-6*kg`*m`/(A`**2*s`**2))

ただし G. Giorgi が SI 単位系の元になった MKSA 単位系を定めたのは 1902 年のことであり、まだ特殊相対論も量子論もなかった時代です。当然微細構造定数も知られていません。この時代に磁気定数 μ0 を「電磁系での世界の力から、機械系の世界での力への変換定数」として認識するのは無理があります。磁気定数は単なる辻褄合わせによって導いたのだと思います。電気技術者だった G. Giorgi にとって、単位系は基本単位系と電磁理論から循環論法にちいることなく、論理の階段を積み上げて定義していくものではなかったのだと思います。お実際 G. Giorgi は次のように主張しています。[文献 1 より](#doc_1)。辻褄合わせで電磁気を理解していた彼には、μ0,ε0 が本当に ether に関係した物理量だと見えていたのだと思います。
...
そして SI 単位系規格書での電流の定義は G. Giorgi の電流の定義を踏襲したままなので、上の分かりにくい:辻褄あわせによる単位電流とμ0 の定義になっているのだと考えます。SI 規格書にある単位電流の定義ではなく、「この節でのような物理的意味を伴った単位電流・磁気定数の定義に修正すべきだ」、「誤解を招くだけの「真空の透磁率／自由空間の透磁率」の用語は廃止すべきだ」と私は主張します。
...
電気・回路技術者にとって電圧 V は基本単位であって、SI 単位系で示される kg m^2/(A s^3) のような組み立て単位ではありません。実際「100 kg m^2/(A s^3) の電圧」といっても電気・回路技術者には通じません。彼らにとっては電圧も基本単位だからです。組み立て単位ではないからです。「100V の電圧」と言わねばなりません。

電気・回路技術者にとって抵抗:Ω は V/A と表される組み立て単位であって、SI 単位系で示される kg m^2/(A^2 s^3) のような組み立て単位ではありません。「100 kg m^2/(A^2 s^3) の抵抗」といっても電気・回路技術者には通じません。「100V/A の抵抗」と言えば、怪訝な顔はするでしょうが通じます。「100Ωの抵抗」と言ってあげるべきですが。

その他の回路素子でも同様です。コイルは V=L dI/dt の関係で動く素子であり V A/s の組み立て単位を持ちます。コンデンサは \\(V = C \int I dt\\) の関係で動く素子であり V A s の組み立て単位で表されます。その他の素子も M S V A からなる組み立て単位系の世界で電気・回路技術者は理解しています。電気・回路技術者は kg を含まない MSVA の電磁世界の範囲だけで設計作業を完結させています。エネルギーが関係するときは組立単位 V I の ワット:W またはワット時:W hour で考えます。Joule で考えるのは機械系の世界と交わる特別のときだけです。
...
電気・回路技術者に限られます。そんな技術者は殆どいないと思います。
...
「D = ε0 E ならば両辺を √(ε0) で割ってやれば」、「B = μ0 H ならば両辺を √(μ0) で割ってやれば」、右辺式と左辺式を同じ組み立て単位の式に変形したことできます。例えば SI 単位系の単位電流は √(ε0) で割ってやることで Heaviside-Lorentz 化された電流値と その組み立て単位になります。下のような具合です。「SI 単位系の 1 A 単位電流は Heaivside-Loretz 化された単位系での単位電流 Al` の 336066.818766 倍と同じ電流量である」といった具合です
    ...
    SI 単位系を Heaviside Lorentz 化したときの単位電場≡単位電圧/単位長さ;;tt(); Vl`/ut.m
    ===============================
...
1. e0n の無次元性：機械系の単位力と電磁系の単位力の比であること
2. u0n が 1/光速度^2の次元を持つ(Maxwell 方程式で、明示的に 1/c^ を外に出してやることで u0n の次元を 1 にできる)ことが

が理解可能でした。
(逆に機械系の単位力と、単位距離だけ離れた電線間に働く力を同じになるように基本単位電流を選択してやれば、u0 を 1 とし e0 を 1/光速度^2とする単位系を構築できました。)

このような機械系の単位力と電磁系の単位力が同じ力が同じになる単位系を選択してやれば

1. 物理的には同じものである真空中の (E,D), (B,H) は、単位系の冗長性の故に組み立て単位が異なるけれど同じになること
1. e0,u0 が 1/c^2 の分を除けば本質的に無次元の 1 であること
...
まず下に電磁気に関する年表を示します。19 世紀は Volta の電推から始まり、 有線モールス通信、Maxwell 方程式、電話、エジソンの白熱電球、発電機、電子の発見、特殊相対論まで 105 年間で突き進んだ凄まじい進歩・変革の時代でした。
...
|1836 年  |ダニエル電池の発明                                                                            |
|1841 年  |J. P. Joule の法則：Q= V A t の発見                                                           |
|1845 年  |J. P. Joule 仕事当量の測定                                                                    |
...
|1867 年  |明治維新：大政奉還                                                                            |
|1872 年  |BAU が実用単位系における抵抗の単位を ohm と呼ぶことにした。                                                    |
...
|1902 年  |Giovani. Giorgi が MKS に A を追加した四元有理化電磁単位系を提唱した                          |
...
- ※2  この時には未だ抵抗・電圧・電流の単位が定まっておらず計算間違いで一回目の大西洋横断ケーブルの敷設は失敗した。このためイギリス政府が Mawell に実用単位を定めることを依頼した。
...
もし Maxwell が「emu 単位系での単位電圧の 10^9倍、単位抵抗の 10^9 倍、単位電流の 10^0 倍を実用単位とする」ことにしていたら、現在でも CGS 単位系が主流だったでしょう。V や Ω は emu 基本単位の 1ギガ倍の値の実用単位となったでしょう。現在のトンの単位のような扱いになっていたでしょう。
...
2. ohm または ampere どちらかを四つ目に付加する基本単位としている。一つに絞っていない。
...
>拙訳:パラメータμ0 は計測体系中に出てくる定数にすぎません。実際に計られる物理量ではありません。いかなる意味でも μ0 は真空の物理的意味を記述するものではありません。SI単位系の標準委員会は、μ0 の用語として物理的意味を暗示しない磁気定数の用語に変更しています。
...
が分っていないことに対する感性の高い学生ほど悩まされます。一方で「真空の誘電率・透磁率とは何なんだ」と悩むことなく電磁気学の教科書を辻褄合わせだけで理解したとする者も多く輩出します。「ε0,μ0 は、本質的に物理次元の無い、しかし機械系の世界と電磁系の世界の間を橋渡しする率:rate である」ことまで理解・到達できる学生は殆どいません。この問題を少しでも避けるため、真空の誘電率・透磁率の用語を電気定数・磁気定数に変更する動きになっています。

>基本単位：MKS に A:単位電流を基本単位として追加すると、単位系としては冗長な単位系になってしまいます。Gauss 単位系、emu 単位系など MKS だけから電磁単位系を構築できていたのですから。それでも MKSA 四元単位系にする理由は、整数べき乗だけの組み立て単位系にできるからです。 1/2 のべき乗の項が出てこなくできるからです。<br>
<br>
　でも冗長な単位系にすることで一つの物理量を複数の組み立て単位で表せてしまいます。力の組み立て単位を K M S^-2 とすると同時に、クーロンの法則からも力の単位を組み立てられます。 (A S)^2 M^-2 も力の組み立て単位になってしまいます。ε0 は、この二つの組み立て単位の間の換算定数です。ですから ε0 は (A S)^2 M^-2/(K M S^-2)==A^2 S^4 K^-1 M^-3 の組み立て単位を持ちます。同様に μ0 は二本の電線の間に働く単位長さ当たりの力の換算定数であり (K M S^-2/M)/(A^2/M)== K M^-1 S^-2 A^-2  の組み立て単位を持ちます。。

このような ε0,μ0 の説明文を SI 単位系の規格書の最後に追加すべきだと私は主張します。皆様はどう考えますでしょうか？

##<a name="proper_MKSA_unit_system"> 本来あるべき SI:MKSA 単位系の定め方

SI 単位系は、Maxwell が定めた実用単位との妥協の産物でした。その妥協手段としてイタリア人の電気技術者 G. Giorgi が提唱した単位系を そのまま採用しました。でも SI 単位系を採択した 1960 年には相対論も量子力学も出来上がっていました。私は G. Giorgi の定めた電流定義を少しだけ変形すべきだったと主張します。

G. Giorgi は平行銅線に働く単位長さ辺りの力を定める式として \\(F = \mu_0 \frac{I_1 I_2}{2\pi}\\) を採用しました。SI 単位系でも その式を無批判に踏襲しました。でも私は相対論を理解していたはずの学者たちならば \\(F = \frac{1}{c^2} U_0 \frac{I_1 I_2}{2\pi}\\) と光速度の入った式を使うべきだったと主張します。そして この \\(U_0\\) の名前を単純に調整定数:fixing constant とすべきでした。磁気定数や真空の透磁率のような無用な誤解を招く名前は避けるべきでした。ε0 は導入せずに \\(\frac{1}{U_0}\\) を使うべきでした。

 \\(F = \frac{1}{c^2} U_0 \frac{I_1 I_2}{2\pi}\\) と \\(U_0\\) を採用するならば  \\(U_0\\) が本質的に無次元の物理量であることが、より明白に見て取れます。本来は 1 であるべき所に Maxwell の定めてしまった単位電流量に妥協した \\(U_0\\) であることを理解できます。
 
妥協したことに伴う調整定数は一つだけで済ますべきです。二つ目の調整定数 ε0 を導入するなんて、物事を複雑にするだけです。

このように \\(\mu_0\\) と \\(\epsilon_0\\) ではなく \\(U_0\\) を導入するだけでも、現在の SI 単位系と同じ組み立て単位を構築できます。ただし私だったら \\(B\\):T:テスラの組み立て単位は導入せずに \\(H\\) だけで済ませます。 モノポールは存在せず、\\(B\\) と \\(H\\) は本質的に同じものなのですから。そして透磁率は無次元量とします。
 

 妥協した歪な単位系を採用するならば、それを元に記述される Maxwell 方程式も下のように歪な形式であるべきです。この方が 1A 実用単位電流に妥協した物理的意味が明確になると思います。ε0,μ0 と二つも変数を導入してやれば1A 実用単位電流に妥協した歪さを消せます。対称な Mawell 方程式にできてしまいます。でも それでは物理的な意味が不明瞭になってしまうと主張します。実際に そのような Maxwell 方程式で学んだ学生の大部分が微細構造定数の物理的な意味を理解できていません。
...
\\]
\\[
  {\nabla}\times {E}= -U_0 \frac{\partial{H}}{\partial t}
\\]
\\[
  {\nabla}\times {H} = {j} + \frac{1}{U_0}\frac{\partial{E}}{\partial t}
\\]
...
また この文章もしくは sfCrrntIniRelativity.py/sfCrrntIniOpRelativity.py に誤りがありましたら教えてやってください。
