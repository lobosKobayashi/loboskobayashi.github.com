...
"これにより" より後ろの文章は無茶苦茶です。磁気定数とはなんですか。自由空間の透磁率とは何ですか。論理が飛躍しすぎです。しかも IS 単位系の規格書には、磁気定数や自由空間の透磁率の言葉はここに出てくるだけで、その定義・説明がありません。

もっと基本に立ち返ります。ガウス単位系では ε0・μ0 は無次元の 1 の物理量でした。無次元だった物理量が、基本単位系のとり方を変えるだけで、MKSA 単位系に変えるだけで次元をもった物理量に変わるなんてありえないでしょう。自然法則は人間が採用する単位系に依存することなく同じように記述できるはずです。

さらに基本的なことを言います。そもそも透磁率という率の付いた物理量は無次元の物理量に使う言葉です。物理の次元をもった μ0 には透磁度の言葉を使うべきです。こんなことは電磁気学学の教科書を書くような学者ならば 100 も承知のはずです。根本的な何かが見逃されてそうです。<!-- 英文では省略する。 gaus system of units での透磁率・誘電率が無次元であることを説明することは上でなされている。-->
...
ただし、この論考を進めるには、SI 単位系は歴史的経緯のために複雑になりすぎています。より単純で見通しの良い自然単位系を主に使います。幸いにも PythonSf では「ユーザー・カスタマイズ例として配布している Python モジュール・ファイル sfCrrntIniRelativity.py sfCrrntIniOpRelativity.py」に、自然単位系やプランク単位系を実装しています。これを使うことで、単位系の考察に出てくる数式を実際に計算可能なものにできます。論考を数値実験で確認していけます。

そもそも循環論法に陥ることなく電磁系の世界の組み立て単位全てを基本単位の組み合わせの連鎖として構築するには、電磁系の世界は複雑すぎます。コンピュータの助けを借りて初めて理解可能な複雑さになっています。
そして、SI 単位系・自然単位系・Planck 単位系・Heaviside-Lorentz 単位系全体を矛盾無く計算可能なプログラムを設計・実装することは、電磁単位系を正く理解するための絶好の演習問題になっています。循環論法に目を瞑ったまま、辻褄合わせで電磁気の単位系を理解できた気になっているままで、この四つの電磁単位系のプログラムを実装すると、プログラムに循環参照ができてしまいます。無理に動かしても未定義エラーとなってしまいます。電磁単位系全体を正しく理解していないと、この四つの電磁単位系を実際に矛盾なく動作させるプログラムを実装できません。
...
数値計算のためだけでなく、電磁単位系を考察していく際にも自然単位系が合理的です。SI 単位系では天下りに与えられた・または辻褄あわせでしか導けなった ε0,μ0 の物理次元が、Coulomb の法則・電磁誘導の法則から物理的必然として明晰に導かれます。SI:MKSA 単位系では物理的意味の不明な電圧組み立て単位：kg m^2/(A s^3), 抵抗組み立て単位：kg m^2/(A^2 s^3) を、辻褄あわせではなく、物理的な意味を伴った必然の物理量として組み立て・導出できます。
...
自然単位系の物理量を toSI(..) 関数の引数値に与えて呼び出すと SI 単位系での物理量値が返ってきます。下のような具合です
...
以下の考察は実際に PythonSf で計算可能な式を使って行っていきます。皆様もぜひ自分でも計算しながら読んでみてください。自分で自分自身に問題提義しながら、ここでは書いていないことも含めて、御自分の頭でも考えながら読み進めてください。誤りがあったら、コンピュータの計算結果が想定外の値を返すので容易に気付けます。単位付きの加減乗除算を行うだけですから、[PythonSf マニュアル](http://loboskobayashi.github.io/pysf/manual/readMeOfPythonSf.htm)の詳細を読まなくても[ vim エディタで数式文字列を入力できさえすれば計算できます](http://loboskobayashi.github.io/pysf/manual/readMeOfPythonSf.htm#%E2%96%A0%E2%96%A0%20PythonSf%20vim%20operation)。
...
大多数の電気・回路技術者は、そして物理学者の多くも、上で述べたような基本単位電荷から電圧の組み立て単位の導出を行っていません。MKSA 単位系では、電荷ではなく電流を基本単位とるるからです。電流を基本単位とする単位系では上のように ε0 を組み立てるのは困難だからです。ですから、100 kg m^2/(A s^3) の電圧と言われても 100V の意味だと理解できません。

普通のエンジニアに「100 kg m/s^2/m^2 の圧力」「100 N/m^2 の圧力」「100 Pa の圧力」いずれの組み立て単位で圧力を表現しても通じるでしょう。もし「100 kg m/s^2/m^2 の圧力」で通じなかったら、彼らは圧力という物理概念を理解していないことを意味します。でも「100 kg m^2/(A s^3) の電圧」が通じない技術者・物理学者が電圧の概念を理解していないわけではありません。彼らは電圧を組立単位ではなく基本単位として理解しているからです。彼らは質量ではなく電圧を基本単位に使う MSAV 四元単位系で閉じた電磁系の世界で理解・検討・設計しているからです。彼らにとって抵抗の組立単位は kg m^2/(A^2 s^3) ではなくV/A だからです。この詳細は[PythonSf MKS + A V 単位系](#MKSAV_unit_system)の章で論考します。
...
まず下に電磁気に関する年表を示します。19 世紀は Volta の電推から始まって 105 年間に 有線モールス通信、Maxwell 方程式、電話、エジソンの白熱電球、発電機、電子の発見、特殊相対論まで突き進んだ凄まじい進歩・変革の時代でした。
