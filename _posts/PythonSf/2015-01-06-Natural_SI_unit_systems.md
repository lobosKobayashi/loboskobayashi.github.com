...
私に言わせれば "これにより" より後ろの文章は無茶苦茶です。磁気定数とはなんですか。自由空間の透磁率とは何ですか。論理が飛躍しすぎです。しかも SI 単位系の規格書には、磁気定数や自由空間の透磁率の言葉はここに出てくるだけで、その定義・説明がありません。 また μ0 の組み立て単位は Newton/A^2 とすべきです。この単位電流の定義を元に、H：ヘンリーも含めた組み立て単位を構築しなければならないのに、電流という基本単位を定義している段階で H という組立単位を入れてくるなんて循環論法です。
...
###<a name="drivation_of_Volt_drived_unit"> 電圧の単位：kg m^2/(A s^3) の導出
