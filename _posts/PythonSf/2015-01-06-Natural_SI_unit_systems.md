...
私に言わせれば "これにより" より後ろの文章は無茶苦茶です。磁気定数とはなんですか。自由空間の透磁率とは何ですか。論理が飛躍しすぎです。しかも SI 単位系の規格書には、磁気定数や自由空間の透磁率の言葉はここに出てくるだけで、その定義・説明がありません。 μ0 の単位は Newton/A^2 とすべきです。この単位電流の定義を元に、H：ヘンリーも含めた組み立て単位を構築しなければならないのに、定義の段階で組立単位を入れてくるなんて循環論法です。
...
ただし この論考を進めるには、SI 単位系は不適切です。電磁気自体の複雑さの上に、さらに歴史的経緯による複雑が重なってしまっているからです。より単純で見通しの良い自然単位系を主に使います。幸いにも PythonSf では「ユーザー・カスタマイズ例として配布している Python モジュール・ファイル sfCrrntIniRelativity.py sfCrrntIniOpRelativity.py」に、自然単位系やプランク単位系を実装しています。これを使うことで、単位系の考察に出てくる数式を実際に計算可能なものにできます。論考を数値実験で確認していけます。
...
G. Giorgi は平行銅線に働く単位長さ辺りの力を定める式として \\(F = \mu_0 \frac{I_1 I_2}{2\pi}\\) を採用しました。SI 単位系でも その式を無批判に踏襲しました。でも私は相対論を理解していたはずの学者たちならば \\(F = \frac{1}{c^2} U_0 \frac{I_1 I_2}{2\pi}\\) と光速度の入った式を使うべきだったと主張します。そして この \\(U_0\\) の名前を単純に調整定数:fixing constant とすべきでした。磁気定数や真空の透磁率のような無用な誤解を招く名前は避けるべきでした。ε0 は導入せずに \\(\frac{1}{U_0}\\) を使うべきでした。
 ...
 \\(F = \frac{1}{c^2} U_0 \frac{I_1 I_2}{2\pi}\\) と \\(U_0\\) を採用するならば  \\(U_0\\) が本質的に無次元の物理量であることが明白に見て取れます。本来は 1 であるべき所に Maxwell の定めてしまった単位電流量に妥協した \\(U_0\\) であると理解できます。妥協したことに伴う調整定数は一つだけで済ますべきです。二つ目の調整定数 ε0 を導入するなんて、物事を複雑にするだけです。
