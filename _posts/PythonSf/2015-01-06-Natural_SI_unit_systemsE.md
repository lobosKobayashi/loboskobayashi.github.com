...
title : secrets of ε0,μ0： considerations of SI system of units from a natural unit system--Eng
---
{% include JB/setup %}
<!--
...
1. ε0・μ0 are **1** without units in nature. But if you chose the MKSA system of units, then ε0,μ0 become not 1 physical quantities with units. 
...
1. The unnecesary physical constants:ε0 and μ0 have brought about "electric field and dielectric flux" or "magnetic field and magnetic flux" of different value and different derived units even in vacuum. Though they should be same values and same derived units. 
1. If you chose  [Heaviside-Lorentz](#Heaviside_Lorentz_ing) system of units, then these physical quantities become same value and same derived units.
...
論考を実際に動くプログラム動作で確認できることは、論考全体が正しいことを強く示唆してくれます。
一箇所でも論考に矛盾が入り込んだら、プログラム動作に矛盾が生じてくるからです。
...
<!---
### 綺麗な Heaviside-Lorentz 単位系での 抵抗・電池回路の相対論的：<A,`i φ> 解釈

後で電荷/長さが電圧の物理量であることを Heaviside-Lorentz 単位系で使います。

物理学科の学生でもベクトル・ポテンシャルの組み立て単位を即答できるものは少ない
のが現実でしょう。
でも、物理現象を表す式を書けても、その式に出てくる物理量の組み立て単位を即答で
きないことは、その物理現象を不完全にしか理解できていないと私は主張します。

ベクトル・ポテンシャルは rot B == 0 より B == rot A となる量として電磁気学では
導入されます。でも、Lorentz 変換を理解しているならば、静止電荷が作る静電ポテン
シャル φを <0,0,0,`iφ> 四元ポテンシャルとして解釈し、その Lorentz 変換、動く
座標系から見たとき 0 でなくなる空間成分として解釈したほうが、その物理的実在
性・必然性・直感的理解の面で勝ります。

√N=V=C/m
# <A,φ/c> 四元ベクトルを組み立て単位付きで物理的意味を解釈する
# ベクトル・ポテンシャル組み立て単位の MKSV:Heaviside-Lorentz 単位系 → MKSC or MKSA 単位系変換
V=√N == q/m 時間成分 vs 空間成分 == q v/c /m == 1/c q v/m == 1/c I dl/m

↑ こっちの式ならば物理的必然として、組み立て単位系付きで素直に受け入れられる。
↑ c` 係数の掛かり方も自然だ。
↑導出
↑ 形式的に √N を V で置き換えているが、この V の物理的意味は MKSA or MKSC 単位系での V とは物理的意味が異なる
    ↑ 電磁系世界での準基本単位
        ↑ この V のもって旨い説明もしくは深い物理的解釈を御存知の方は教えてください。

## MKSC or MKSA 側は醜い
1/c = √e0 √u0
c^-1 √N == c^-1 q/m  == 1/c^2 I dl/m == e0 u0 I dl/m
(c e0)^-1 √N == (c e0)^-1 q/m        == u0 I dl/m
              == c^-1 e0^-1 q/m        == u0 I dl/m
              == c^-1  q/(e0 m)        == u0 I dl/m     # MKSA 単位系での φ,A 四元ベクトル
↑fitting：適合
    ↑ こんな組み立て単位が直ぐに思い浮かべられないのは当然だろう
    ↑ 斜めから歪んだ姿を見ている

R(x) (φ(x+Δx,t)-φ(x,t))/Δx == I(x)

電線内では A ~== I  # ここは少し自信が無い。

相対論的電磁幾何では速度ベクトル u4 は絶対値 -1 の無次元ベクトル量

q v4 Δt <A,φ> == q Δx4 <A,φ> == ΔEnergy
Q Δl4
I Δl4 == Q v4 <A,φ> == ΔEnergy/Δt == Watt
電池：二種類の物質の間の電位差 V: その V を電子が移動するときに発生するエネルギー ΔE = V ΔQ
ΔE/Δt = Watt = V ΔQ/Δt == V I

導体:電気を通す物質が存在する
電圧は二点間の電位差ではなく、空間電位差の間に連続して分布する物理量
発電機は巨大なイナーシャをもった磁石を回転させたもの。電線に電圧を分布させる装
置。<0,0,0,φ> 分布を作り出す装置。

<0,0,0,φ> 分布を導体に発生させれば Q Δl4 に ΔEnergy を発生させる位置関係を
発生する
↑ 導体物質中では、原子核は結晶格子の位置に束縛されるが、電子の電荷は殆ど自由
に動ける。完全に自由でなくしているのは抵抗という物質量

発電機が作る <0,0,0,φ> を打ち消すように自由電子が動く。その動く自由電子全体は
<Ax,Ay,Az,-φ> 分布を発生させる。電子の四元速度ベクトルは時間成分も発生させ
る。原子核陽子の作る φ分布と打ち消しあう以外の φ 分布を生成する。

電流の組立単位とベクトル・ポテンシャルの組み立て単位が同じであることは電気回路
の考察で重要になります。

交流回路理論では、複素インピーダンスで加減乗除算を行い、実際の電圧・電流は実数
部分だけを採用します。虚数部分は捨てます。
でも虚数部分も物理的実在が存在しているはずです。工学的に利用していませんが。
...
div A + 1/c   ∂ct(φ)==0
## <A,i φ> 分布:相対論的電磁気学
div quote:注意:完全オリジナル

div quote:sfCrrntIniRelativity.py を作った理由
電磁気学は相対論で考えたほうが単純にできることが多くあります。
PythonSf を使って、それらを数値実験できます。
光速度が 1 の単位系を使わないと、コンピュータの計算誤差に正しい計算値が埋もれ
てしまいます。

Coulomb の法則 + Lorentz 変換 ⇔ Maxwell 方程式

### ベクトル・ポテンシャルの組立単位

Heaviside-Lorentz 単位系での電流の組立単位批判

Lorentz 条件

### 回路動作を <A,i φ> の視点で解釈しなおす
E の分布ではなく φ の分布を考える


# 交流回路理論と <A,i φ> 分布:相対論的電磁気学
↑ 複素インピーダンスの理論は、交流電源を与えた後、過度状態を経て安定状態におちついたときの電圧と電流の関係

## 抵抗と電池の組み合わせ回路
抵抗が発熱するのは電池のエネルギーが空間を伝わっていくから。
